...
import { 
  ...
  export class ComponentDatabase {
    ...
    checkCompatibility(setup: DrivetrainSetup): CompatibilityCheck {
      ...
      const speeds = [setup.crankset.speeds || setup.cassette.speeds, setup.cassette.speeds, setup.chain.speeds, setup.rearDerailleur.speeds];
      const uniqueSpeeds = [...new Set(speeds.filter(s => s))];
      ...
      // This would need more complex logic based on actual freehub standards
      notes.push(`Freehub type: ${setup.cassette.freehubType}`);
  
      // Check chain line (basic check)
      if (setup.bikeType === 'road' && setup.crankset.chainLine !== 43.5) {
        warnings.push({
          type: 'warning',
          component: 'crankset',
          issue: `Non-standard chain line: ${setup.crankset.chainLine}mm (standard is 43.5mm)`,
